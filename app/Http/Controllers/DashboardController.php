...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $variacaoReceitas = $receitasAnterior > 0 ? (($totalReceitas - $receitasAnterior) / abs($receitasAnterior)) * 100 : ($totalReceitas > 0 ? 100 : 0);
        $variacaoDespesas = $despesasAnterior > 0 ? (($totalDespesas - $despesasAnterior) / abs($despesasAnterior)) * 100 : ($totalDespesas > 0 ? 100 : 0);
        $variacaoSaldo = $saldoAnterior != 0 ? (($saldo - $saldoAnterior) / abs($saldoAnterior)) * 100 : ($saldo > 0 ? 100 : 0);

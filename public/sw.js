    ...
    event.respondWith(
        fetch(req)
            .then((res) => {
                ...
                // For errors (404, 500, etc), return as-is
                if (res) return res;
                // No response from network
                throw new Error("No response");
            })
            .catch((err) => {
                // Network error: try cache, then offline fallback
                return caches
                    .match(req)
                    .then(
                        (cached) =>
        ...
        req.onupgradeneeded = (e) => {
            ...
            if (!db.objectStoreNames.contains("despesas")) {
                db.createObjectStore("despesas", {
                    keyPath: "client_queue_id",
                });
            }
            if (!db.objectStoreNames.contains("receitas")) {
                db.createObjectStore("receitas", {
                    keyPath: "client_queue_id",
                });
            }
        };
...
async function cacheFirst(req, cacheName) {
    ...
    try {
        ...
        // Se não OK mas é response válida (ex: 404), retornar
        if (res) return res;
        // Se não há response, retornar offline fallback
        return new Response(null, {
            status: 503,
            statusText: "Service Unavailable",
        });
    } catch (err) {
        // Erro de rede: tentar cache, senão offline
        const cached = await caches.match(req);
        if (cached) return cached;
        return new Response(null, {
            status: 503,
...
async function staleWhileRevalidate(req, cacheName) {
    ...
    const fetchPromise = fetch(req)
        ...
        .catch((err) => {
            console.warn("[SW] Fetch error for", req.url, err);
            return null;
        });
...
async function networkFirstNav(req) {
    ...
    try {
        ...
        console.warn("[SW] Network request failed:", req.url, err);
    }
    ...
    // Cache não tem, retornar página offline
